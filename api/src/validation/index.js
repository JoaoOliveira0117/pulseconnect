...
export const queryExists = (prop) => query(prop)
  .exists() 
  .withMessage(`${prop} is required`)

export const paramExists = (prop) => param(prop)
  .exists() 
  .withMessage(`${prop} is required`)

export const userExistsWithProp = (prop) => check(prop)
  .custom(async (_, {req}) => {
    const reqKeys = {...req.body,...req.params, ...req.query}
...
export const postExistsWithProp = (prop) => check(prop)
  .custom(async (_, {req}) => {
    const reqKeys = {...req.body,...req.params, ...req.query}
